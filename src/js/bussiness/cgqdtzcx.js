export default {
  ...
  data() {
    return {
      ...
      formData: {
        companyid: 19940, supplyid: null, goodsid: null, check1: null
      },
      btnOptions: [
        {id: 'query', icon: "fa fa-search", label: "action.search", perms: "business:cgqdtzcx:select"},
        {id: 'download', icon: "fa fa-download", label: "action.download", perms: "business:cgqdtzcx:select"}
      ],
      tableColumns: [
        {type: 2, prop: 'chdate', label: '调整时间', tableStatus: 0, width: 150, widthStatus: true},
        {type: 0, prop: 'ddlname', label: '类型', tableStatus: 0},
  ...
  methods: {
    queryFunction(data) {
      if (data === "query") {
        ...
        this.exportExcelDOM(this.tableColumns, this.tableData, '采购渠道调整查询' + this.getNewDate())
      }
    },
    handleChange(data) {
    ...
    selectDoc(pageNum, pageSize) {
      ...
      this.loading = true;
      this.$api.bussiness.selectCgqdtzcx({pageNum: pageNum, pageSize: pageSize, params: params}).then(res => {
        if (res.code === 200) {
