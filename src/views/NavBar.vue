<template>
    ...
    <div class="logo" :style="{'background':themeColor}" @click="$router.push('/')"
      ...
      <img v-if="collapse"  src="@/assets/logomini.png"/> <div>{{collapse?'':appName}}</div>
    </div>
    <!-- 导航菜单 -->
    <el-menu ref="navmenu" default-active="1" :class="collapse?'menu-bar-collapse-width':'menu-bar-width'"
      ...
      @open="handleopen" @close="handleclose" @select="handleselect" style="overflow-y:auto;overflow-x:hidden;">
      <!-- 导航菜单树组件，动态加载菜单 -->
...
.menu-bar-container {
  ...
  .el-menu {
    ...
    .el-submenu{border-bottom:1px solid #ccc;background:linear-gradient(#CDDEF9, #ECF3F9  60%);font-weight: bold;}
    .el-menu-item{height:30px;line-height:30px;border-bottom:1px solid #ccc;}
  }
  .logo {
    position:absolute;
    top: 0px;
