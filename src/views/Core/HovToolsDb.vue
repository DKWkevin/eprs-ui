<template>
  ...
  <el-dialog :title="hovTitle" :visible.sync="HovVisible" center width="760px" top="10vh" @close="closeHov" data-v-f3f3eg9>
    ...
    <div style="background:#eee;border-bottom:1px solid #ccc;clear:both;padding:5px;">
        ...
        <el-button @click="cellDBLClickEvent()" size="small" style="margin-left:10px;" type="primary">确认回填</el-button>
      </div>
    <div>
      <el-table
      size="mini" 
  ...
  methods:{
    ...
    cellDBLClickEvent(data,event){
      ...
      for(var i=0;i<backfill.length;i++){
        let params=[];
        this.fillArray.forEach(e=>{
          params.push(e[backfill[i].prop]);
        });
        this.$parent[this.fillDataName][backfill[i].fillid]=params.toString();
      }
    ...
    query(){
      ...
      if(this.pageStatus===true){
        hovData={pageNum:this.currentPage,pageSize:this.pagesize,params:formData};
      }else{
        hovData={pageNum:1,pageSize:999999,params:formData};
      }
      this.$api.hov.query(this.hovUrl,hovData).then(function(res){
        if(res.code===200){
    ...
    handlePageChange(page){
      ...
      let hovData=null;
      if(this.pageStatus===true){
        hovData={pageNum:page,pageSize:this.pagesize,params:formData};
      }else{
        hovData={pageNum:1,pageSize:999999,params:formData};
      }
      this.$api.hov.query(this.hovUrl,hovData).then(function(res){
    ...
    handleSizeChange(page){
      ...
      let hovData=null;
      if(this.pageStatus===true){
        hovData={pageNum:this.currentPage,pageSize:page,params:formData};
      }else{
        hovData={pageNum:1,pageSize:999999,params:formData};
      }
      this.$api.hov.query(this.hovUrl,hovData).then(function(res){
...
<style scoped>
  /*.hovTools .el-dialog__wrapper .el-dialog__body {
    padding: 30px 20px;
    color: #606266;
    font-size: 14px;
